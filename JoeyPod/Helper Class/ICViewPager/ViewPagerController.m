...
@property (nonatomic) NSUInteger previousActiveIndex;
...
- (NSNumber *)startFromSecondTab {
    ...
    if (!_startFromSecondTab) {
        ...
        NSLog(@"view pager index %@",[NSNumber numberWithFloat:value]);
      }else
      {
          NSLog(@"else");
      }
...
- (void)selectTabAtIndex:(NSUInteger)index didSwipe:(BOOL)didSwipe {
    ...
    self.previousActiveIndex= self.activeTabIndex;
    ...
    NSLog(@"%lu %lu",(unsigned long)self.activeTabIndex, (unsigned long)self.activeContentIndex);
...
//        self.tabsView.layer.shadowOffset = CGSizeMake(-5, 2);
//        self.tabsView.layer.shadowRadius = 5;
//        self.tabsView.layer.shadowOpacity = 0.3;
   ...
   // NSUInteger index = [self.startFromSecondTab boolValue] ? 1 : 0;
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
     ...
     NSLog(@"%f", value);
    [
     self selectTabAtIndex:value didSwipe:NO];
...
//}
//- (NSUInteger )getPreviousIndex
//{
//    return self.previousActiveIndex;
//}

-(UIView* ) getCurrentTabatIndex:(NSUInteger) index
{
...
- (UIViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    NSLog(@"index again %lu",(unsigned long)index);
